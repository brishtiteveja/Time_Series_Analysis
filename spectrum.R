              ...
              plot = TRUE, na.action = na.fail, ...)
{
    ...
    # substitute any variable in the expression of x, if any and then deparse(turn unevaluated expression into character string
    series <- deparse(substitute(x))
    # convert to time series (default frequency 1) + Extract information on the NA action used to create an object.
    x <- na.action(as.ts(x))
    # get the frequency of the time series
    xfreq <- frequency(x)
    # convert into a row-col matrix
    x <- as.matrix(x)
    # get the number of rows of the matrix x
    N <- N0 <- nrow(x)
    # get the number of columns of the matrix x
    nser <- ncol(x)
    
    # check whether smoothing/rolling average is required
...
y <- ldeaths - mean(ldeaths)
p <- spec.pgram(ldeaths, plot=FALSE)
ylimits <- c(1/10*min(p$spec), 25*max(p$spec))
plot(p, ylim=ylimits, log='yes', ci=0)
df <- p$df
lines(p$freq, p$spec/qchisq(0.975, df), t='l', lty=2, col='green')
lines(p$freq, p$spec/qchisq(0.025, df), t='l', lty=2, col='red')

p <- spec.mtm(y, nw=2, k=3, #taper='sine', 
              jackknife = TRUE, dtUnits = 'year', 
              deltat = 1/12,
              #plot=FALSE
              )
#ylimits <- c(1/10*min(p$spec), 25*max(p$spec))
#plot(p, ylim=ylimits, log='yes', ci=0)
#lines(p$freq, df * p$spec/qchisq(0.975, df), t='l', lty=2, col='green')
#lines(p$freq, df * p$spec/qchisq(0.025, df), t='l', lty=2, col='red')
...
library(multitaper)
resSpec <- spec.mtm(ldeaths, nw=2, k = 3, jackknife = TRUE, dtUnits='year', sineAdaptive = FALSE) 
